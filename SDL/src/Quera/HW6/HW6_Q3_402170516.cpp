...
int main(int argc, char const *argv[])
{
  ...
  double Vx_R2 = rand() % 8 + 1, Vy_R2 = rand() % 8 + 1, X0_R2 = (rand() % mod2_x) + R2, Y0_R2 = (rand() % mod2_y) + R2, R_R2 = 255, G_R2 = 0, B_R2 = 255;
  ...
  while (running)
  {
    while (SDL_PollEvent(&event))
    {
      if (event.type == SDL_KEYDOWN)
      {
        ...
        if (key == SDLK_UP || key == SDLK_SPACE)
          Vx_R1 *= 1.5, Vy_R1 *= 1.5, Vx_R2 *= 1.5, Vy_R2 *= 1.5;
        if (key == SDLK_DOWN)
          Vx_R1 *= 0.5, Vy_R1 *= 0.5, Vx_R2 *= 0.5, Vy_R2 *= 0.5;
        if (key == SDLK_i)
    ...
    if (!paused)
    {
      ...
      filledEllipseRGBA(renderer, X0_R1, Y0_R1, R1, R1, R_R1, G_R1, B_R1, 255);
      filledEllipseRGBA(renderer, X0_R2, Y0_R2, R2, R2, R_R2, G_R2, B_R2, 255);
      ...
      if (X0_R1 >= WIDTH - R1 || X0_R1 <= R1)
      {
        X0_R1 -= Vx_R1;
        Vx_R1 *= -1;
        std::cout << "Vx=" << Vx_R1 << " Vy=" << Vy_R1 << " X=" << X0_R1 << " Y=" << Y0_R1 << std::endl;
      }
      ...
      if (X0_R2 >= WIDTH - R2 || X0_R2 <= R2)
      {
        X0_R2 -= Vx_R2;
        Vx_R2 *= -1;
        std::cout << "Vx=" << Vx_R2 << " Vy=" << Vy_R2 << " X=" << X0_R2 << " Y=" << Y0_R2 << std::endl;
      }
